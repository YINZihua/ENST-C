...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < max_reareader_idders ; i++)  {
...
void writers_function(int writer_id){
  ...
  for (i = 0; i < 3; i++){
    ...
    /*  TO BE COMPLETED */
        //cela a pour objectif d'eviter les conflits de modification du fichier
...
void readers_function(int reader_id) {
  ...
  /* TO BE COMPLETED: protect shared data in is_first  */
      ...
      // cela a pour objectif d'eviter le conflit de lecture entre les differents actionneurs de programme
      if (is_first()) {
        printf("========================= Reader %d is the first one entering the critical section (i.e. accessing the shared resource) \n", reader_id);
        /*****  Begining of the critical section for the readers/writers mechanism *****/
        /*  TO BE COMPLETED */
          //cela a pour objectif d'eviter les conflits de consultations du fichier
          sem_wait(lock_for_shared_variables);
        }


  /* TO BE COMPLETED  */
      //cela a puor objectif d'evieter les conflits entre les differents actionneurs du programme
  ...
  /* TO BE COMPLETED: protect shared data in is_last  */
    //cela a pour objectif d'eviter les conflits entre les differents actionneurs du programme
    sem_wait(lock_for_readers_writers);
  ...
  if (is_last()) {
    ...
    /*****  End of the critical section for the readers/writers mechanism *****/
    /*  TO BE COMPLETED  */
    // cela a pour objectif d'eviter les conflits de la consultations du fichier
      sem_wait(lock_for_shared_variables);
  }
